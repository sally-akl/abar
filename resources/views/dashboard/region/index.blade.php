...
<div class="card">
  <div class="card-header">
    <h3 class="card-title">@lang('site.region')</h3>
  </div>
  <div class="card-body border-bottom py-3">
    ...
    <div class="table-responsive">
      ...
      <table class="table card-table table-vcenter text-nowrap datatable">
        ...
        <tbody>
          ...
          <tr>
            <td>{{$reg->title}}</td>
            <td>{{$reg->gov->country->title}}</td>
            <td>{{$reg->gov->title}}</td>
            <td class="text-right">
              <a class='btn btn-info btn-xs edit_btn' bt-data="{{$reg->id}}">
    						<i class="far fa-edit"></i>
    					</a>
    					<a href="#" class="btn btn-danger btn-xs delete_btn"  bt-data="{{$reg->id}}">
    ...
    <div class="card-footer d-flex align-items-center">
      {{$regions->links('dashboard.vendor.pagination.default')}}
    </div>
  </div>
...
<div class="modal modal-blur fade" id="add_edit_modal" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <form method="POST" action="{{ url('dashboard/region') }}" class="form_submit_model">
...
<script type="text/javascript">
  var _sucess = function(response)
  {
    if(response.sucess)
    {
      ...
      window.location.href = '{{url("/dashboard/region")}}';
    }
    else
    {
      var $error_text = "";
  ...
  $(".edit_btn").on("click",function()
  {
      ...
      var url_edit = '{{url("/dashboard/region")}}'+"/"+id;
      $(".form_submit_model").attr("action",url_edit);

      $.ajax({
          url: '{{url("/dashboard/region")}}'+"/"+id+"/edit",
          type: 'GET',
          dataType: 'json',
          success: function (response) {
            $("input[name='title']").val(response.reg.title);
            $("select[name='country']").val(response.reg.gov.country.id);
            $("select[name='gov']").html(response.governates);
            $("select[name='gov']").val(response.reg.gov_id);
  ...
  $(".delete_it_sure").on("click",function(){
    ...
    var url_delete = '{{url("/dashboard/region")}}'+"/"+id;
