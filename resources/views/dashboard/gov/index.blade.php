...
<div class="card">
  <div class="card-header">
    <h3 class="card-title">@lang('site.gover')</h3>
  </div>
  <div class="card-body border-bottom py-3">
    ...
    <div class="table-responsive">
      ...
      <table class="table card-table table-vcenter text-nowrap datatable">
        ...
        <tbody>
          	@foreach ($governates as $key => $gov)
          <tr>
            <td>{{$gov->title}}</td>
            <td>{{$gov->country->title}}</td>
            <td class="text-right">
              <a class='btn btn-info btn-xs edit_btn' bt-data="{{$gov->id}}">
    						<i class="far fa-edit"></i>
    					</a>
    					<a href="#" class="btn btn-danger btn-xs delete_btn"  bt-data="{{$gov->id}}">
    ...
    <div class="card-footer d-flex align-items-center">
      {{$governates->links('dashboard.vendor.pagination.default')}}
    </div>
  </div>
...
<div class="modal modal-blur fade" id="add_edit_modal" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <form method="POST" action="{{ url('dashboard/governate') }}" class="form_submit_model">
...
<script type="text/javascript">
  var _sucess = function(response)
  {
    if(response.sucess)
    {
      ...
      window.location.href = '{{url("/dashboard/governate")}}';
    }
    else
    {
      var $error_text = "";
  ...
  $(".edit_btn").on("click",function()
  {
      ...
      var url_edit = '{{url("/dashboard/governate")}}'+"/"+id;
      $(".form_submit_model").attr("action",url_edit);
      $.ajax({
          url: '{{url("/dashboard/governate")}}'+"/"+id+"/edit",
          type: 'GET',
          dataType: 'json',
          success: function (response) {
            $("input[name='title']").val(response.title);
            $("select[name='country']").val(response.country_id);
  ...
  $(".delete_it_sure").on("click",function(){
    ...
    var url_delete = '{{url("/dashboard/governate")}}'+"/"+id;
