...
<div class="card">
  <div class="card-header">
    <h3 class="card-title">اراء العملاء</h3>
  </div>
  <div class="card-body border-bottom py-3">
    ...
    <div class="table-responsive">
      ...
      <table class="table card-table table-vcenter text-nowrap datatable">
        ...
        <tbody>
          	@foreach ($oponions as $key => $opion)
          <tr>
            <td>{{$opion->customer_name}}</td>
            <td>{{$opion->country_name}}</td>
            <td>{{$opion->cutomer_opionion}}</td>
            <td class="text-right">
              <a class='btn btn-info btn-xs edit_btn' bt-data="{{$opion->id}}">
    						<i class="far fa-edit"></i>
    					</a>
    					<a href="#" class="btn btn-danger btn-xs delete_btn"  bt-data="{{$opion->id}}">
    ...
    <div class="card-footer d-flex align-items-center">
      {{$oponions->links('dashboard.vendor.pagination.default')}}
    </div>
  </div>
</div>
@endif
...
<div class="modal modal-blur fade" id="add_edit_modal" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <form method="POST" action="{{ url('dashboard/opionion') }}" class="form_submit_model">
        <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control" rows="3" name="cutomer_opionion"></textarea>
          </div>
...
<script type="text/javascript">
  var _sucess = function(response)
  {
    if(response.sucess)
    {
      ...
      window.location.href = '{{url("/dashboard/opionion")}}';
    }
    else
    {
      var $error_text = "";
  ...
  $(".add_btn").on("click",function(){
      $("input[name='method_type']").val("add");
      $(".form_submit_model").attr("action",'{{url("/dashboard/opionion")}}');
      $('#add_edit_modal').modal('show');
      return false;
  });
  $(".form_submit_model").submit(function(e){
      ...
      if(formData.get("method_type") == "edit")
      {
          ...
          var data = {
             ...
             cutomer_opionion : $("textarea[name='cutomer_opionion']").val() ,
          };
          $.ajax({
              type: $method_is,
              url: submit_form_url,
  ...
  $(".delete_it_sure").on("click",function(){
    ...
    var url_delete = '{{url("/dashboard/opionion")}}'+"/"+id;
